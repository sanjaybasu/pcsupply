...
# Download raw IHME ca files ---------------------------------------
# See https://github.com/BuzzFeedNews/2017-05-us-health-care/blob/master/index.Rmd

url = "http://ghdx.healthdata.org/sites/default/files/record-attached-files/IHME_USA_COUNTY_CANCER_MORTALITY_RATES_1980_2014_NATIONAL_XLSX.zip"
...
raw_src = "data-raw/county/IHME_USA_COUNTY_ca_MORTALITY_RATES_1980_2014_NATIONAL_Y2017M05D16.XLSX" # Raw data
ca_us <- read_excel(raw_src, sheet = 1, skip = 3, col_names = FALSE) %>%
  select(1:10)
names(ca_us) <- c("place","fips","ca_1980","ca_1985","ca_1990","ca_1995","ca_2000","ca_2005","ca_2010","ca_2014")
...
ca_counties = ca_counties[1:3142,]
...
states_names <- ca_counties %>%
  select(1:3) %>%
  inner_join(states) %>%
  mutate(place = paste0(place,", ",abb))

# Recombine AHRF with processed life expectancy data ---------------------------------------
ca_counties <- bind_cols(states_names,counties_clean) 


# Linearly impute 2015 LE ----
ca_counties$ca_2015 = ca_counties$ca_2014 + 1/4*(ca_counties$ca_2014-ca_counties$ca_2010)

# keep relevant years ----
ca_counties <- ca_counties %>%
  select(fips, ca_2005, ca_2010, ca_2015)
...
counties_data <- left_join(ahrf_county,ca_counties, by=c("fips"="fips"))
counties_data
...
# Reshape wide to long; note that guam and puerto rico don't have ca available and make up most of the NA's, so need to exclude them when counting NA's for 50 states+DC ----
...
plot(panel$pc,panel$ca)
plot(log(panel$pc),(panel$ca), xlab = "Log (# of primary care physicians/10k pop)", ylab = "Age-adjusted life expectancy at birth (years)")
...
zpanel$ca = zpanel$ca*10



reg_base = (lmer(ca~pc+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval+ (1+pc| county)+ (1|time) ,
                 data = zpanel))
...
reg_melu = lmer(ca~pc+pc*urb+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
...
reg_melp = lmer(ca~pc+pc*pov+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
...
reg_melb = lmer(ca~pc+pc*blk+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
...
reg_melh = lmer(ca~pc+pc*his+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
