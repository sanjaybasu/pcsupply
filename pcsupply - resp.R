...
ahrf_county %>% 
  select(county = F04437, 
         ...
         mrespre_2011 = `F15549-11`, #  Mdcre Enrllmnt, Agd & Dsbld Tot,  CMS Enroll Dashboard     
         mrespre_2015 = `F15549-15`
  ) %>% 
  mutate(pop_2005 = as.integer(pop_2005),
         ...
         mrespre_2005 = (as.integer(mrespre_2011)-6/4*(as.integer(mrespre_2015)-as.integer(mrespre_2011)))/pop_2005*100,
         mrespre_2010 = (as.integer(mrespre_2011)-1/4*(as.integer(mrespre_2015)-as.integer(mrespre_2011)))/pop_2010*100,
         mrespre_2015 = as.integer(mrespre_2015)/pop_2015*100
  ) -> ahrf_county
...
reg_pool = plm(resp~pc+
                 urb+ed+medct+eld+fem+blk+his+unemp+poll+inc+pov+hobed+unins+mrespre+obese+tob+spec,
...
reg_fd =  plm(resp~pc+
                urb+ed+medct+eld+fem+blk+his+unemp+poll+inc+pov+hobed+unins+mrespre+obese+tob+spec+y00+y05+y10,
