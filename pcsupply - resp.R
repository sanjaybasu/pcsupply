...
# Reshape wide to long; note that guam and puerto rico don't have resp available and make up most of the NA's, so need to exclude them when counting NA's for 50 states+DC ----
...
plot(panel$pc,panel$resp)
plot(log(panel$pc),(panel$resp), xlab = "Log (# of primary care physicians/10k pop)", ylab = "Age-adjusted life expectancy at birth (years)")
...
zpanel$resp = zpanel$resp*10



reg_base = (lmer(resp~pc+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval+ (1+pc| county)+ (1|time) ,
                 data = zpanel))
...
reg_melu = lmer(resp~pc+pc*urb+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
...
reg_melp = lmer(resp~pc+pc*pov+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
...
reg_melb = lmer(resp~pc+pc*blk+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
...
reg_melh = lmer(resp~pc+pc*his+spec+urb+pov+ed+fem+blk+his+unemp+hobed+mcare+medct+tob+obese+poll+homeval + (1+pc| county)+ (1|time) ,
                data = zpanel)
