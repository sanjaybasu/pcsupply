...
doc_src = "data-raw/county/AHRF 2016-2017 Technisubstl Documentation.xlsx"
...
# Download raw IHME subst files ---------------------------------------
# See https://github.com/BuzzFeedNews/2017-05-us-health-substre/blob/master/index.Rmd
...
subst_us <- read_excel(raw_src, sheet = 1, skip = 3, col_names = FALSE) %>%
  select(1:10)
names(subst_us) <- c("place","fips","subst_1980","subst_1985","subst_1990","subst_1995","subst_2000","subst_2005","subst_2010","subst_2014")
subst_us2 <- read_excel(raw_src, sheet = 2, skip = 3, col_names = FALSE) %>%
  select(1:10)
names(subst_us2) <- c("place","fips","subst_1980","subst_1985","subst_1990","subst_1995","subst_2000","subst_2005","subst_2010","subst_2014")
subst_us3 <- read_excel(raw_src, sheet = 3, skip = 3, col_names = FALSE) %>%
  select(1:10)
names(subst_us3) <- c("place","fips","subst_1980","subst_1985","subst_1990","subst_1995","subst_2000","subst_2005","subst_2010","subst_2014")
subst_us4 <- read_excel(raw_src, sheet = 4, skip = 3, col_names = FALSE) %>%
  select(1:10)
names(subst_us4) <- c("place","fips","subst_1980","subst_1985","subst_1990","subst_1995","subst_2000","subst_2005","subst_2010","subst_2014")
...
subst_counties <- subst_us %>%
  filter(grepl(",",place)) %>%
  separate(place, into=c("place","state"), sep = ", ")
subst_counties2 <- subst_us2 %>%
  filter(grepl(",",place)) %>%
  separate(place, into=c("place","state"), sep = ", ")
subst_counties3 <- subst_us3 %>%
  filter(grepl(",",place)) %>%
  separate(place, into=c("place","state"), sep = ", ")
subst_counties4 <- subst_us4 %>%
  filter(grepl(",",place)) %>%
  separate(place, into=c("place","state"), sep = ", ")

subst_counties = subst_counties[1:3142,]
subst_counties2 = subst_counties2[1:3142,]
subst_counties3 = subst_counties3[1:3142,]
subst_counties4 = subst_counties4[1:3142,]


# Select columns with life expectancy data and convert from text string with confidence intervals to numbers ---------------------------------------
counties_clean <- subst_counties %>%
  select(4:11) %>%
  mutate_all(funs(as.numeric(substring(.,1,5))))
counties_clean2 <- subst_counties2 %>%
  select(4:11) %>%
  mutate_all(funs(as.numeric(substring(.,1,5))))
counties_clean3 <- subst_counties3 %>%
  select(4:11) %>%
  mutate_all(funs(as.numeric(substring(.,1,5))))
counties_clean4 <- subst_counties4 %>%
  select(4:11) %>%
  mutate_all(funs(as.numeric(substring(.,1,5))))
...
counties_clean_tot = counties_clean+counties_clean2+counties_clean3+counties_clean4

# Join that to the counties' names, states, and FIPS codes ---------------------------------------
states_names <- subst_counties %>%
  select(1:3) %>%
  inner_join(states) %>%
  mutate(place = paste0(place,", ",abb))

# Recombine AHRF with processed life expectancy data ---------------------------------------
subst_counties <- bind_cols(states_names,counties_clean_tot) 


# Linearly impute 2015 LE ----
subst_counties$subst_2015 = subst_counties$subst_2014 + 1/4*(subst_counties$subst_2014-subst_counties$subst_2010)

# keep relevant years ----
subst_counties <- subst_counties %>%
  select(fips, subst_2005, subst_2010, subst_2015)

# Save LE  ----------------------------------------------------------------

save(subst_counties,file="subst_counties")
...
counties_data <- left_join(ahrf_county,subst_counties, by=c("fips"="fips"))
counties_data
...
panel$resp = panel$subst*10


# Regressions  ----

library(lme4)
reg_meu = lmer(subst~pc+ (1+pc| county)+ (1|time) ,
               data = panel)
summary(reg_meu)
confint(reg_meu,method="Wald")

reg_mes = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
               data = panel)
summary(reg_mes)
...
reg_mel = lmer(subst~lagpc1+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+lagpc1| county)+ (1|time) ,
               data = dd)
summary(reg_mel)
...
reg_mef = lmer(subst~fp+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
               data = panel)
summary(reg_mef)
...
reg_meg = lmer(subst~gim+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
               data = panel)
summary(reg_meg)
...
reg_meu = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
               data = panelurb)
summary(reg_meu)
confint(reg_meu,method="Wald")

panelrur = panel[panel$urb==0,]
reg_mer = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
               data = panelrur)
summary(reg_mer)
...
reg_melp = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = panellopov)
summary(reg_melp)
confint(reg_melp,method="Wald")

panelhipov = panel[panel$pov>(-0.1584),]
reg_mehp = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = panelhipov)
summary(reg_mehp)
...
reg_melb = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = panelloblk)
summary(reg_melb)
confint(reg_melb,method="Wald")

panelhiblk = panel[panel$blk>(-0.47508),]
reg_mehb = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = panelhiblk)
summary(reg_mehb)
...
reg_melh = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = panellohis)
summary(reg_melh)
confint(reg_melh,method="Wald")

panelhihis = panel[panel$his>(-0.38146),]
reg_mehh = lmer(subst~pc+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1|time) ,
                data = panelhihis)
summary(reg_mehh)
