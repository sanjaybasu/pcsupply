...
reg_base = (lmer(infect~pc+spec+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob + (1+pc| county)+ (1|time) ,
                 data = zpanel))
summary(reg_base)
...
reg_meu = lmer(infect~pc+urb+pc*urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
               data = zpanel)
summary(reg_meu)
...
reg_melp = lmer(infect~pc+pc*pov+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = zpanel)
summary(reg_melp)
confint(reg_melp,method="Wald")


reg_melb = lmer(infect~pc+pc*blk+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = zpanel)
summary(reg_melb)
confint(reg_melb,method="Wald")


reg_melh = lmer(infect~pc+pc*his+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = zpanel)
summary(reg_melh)
