...
reg_base = (lmer(cvd~pc+spec+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob + (1+pc| county)+ (1|time) ,
                 data = zpanel))
summary(reg_base)
...
reg_meu = lmer(cvd~pc+urb+pc*urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
               data = zpanel)
summary(reg_meu)
...
reg_melp = lmer(cvd~pc+pc*pov+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = zpanel)
summary(reg_melp)
confint(reg_melp,method="Wald")


reg_melb = lmer(cvd~pc+pc*blk+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = zpanel)
summary(reg_melb)
confint(reg_melb,method="Wald")


reg_melh = lmer(cvd~pc+pc*his+urb+ed+medct+fem+blk+his+unemp+poll+pov+hobed+mcare+obese+tob+spec + (1+pc| county)+ (1|time) ,
                data = zpanel)
summary(reg_melh)
